...
| Diagnóstico por repo: deuda técnica, duplicados, reingeniería, código muerto | **`get_project_analysis`** | `list_known_projects` → **`roots[].id`** del repo objetivo → `get_project_analysis(projectId, mode)` (parámetro MCP = id de repo) |
