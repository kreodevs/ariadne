...
Si el proyecto tiene **dependencias a dominios** en ingest, el sistema puede consultar también grafos Falkor de otros proyectos en esos dominios. Las herramientas usan **`cypherShardContexts`** (`graphName` + `cypherProjectId` por shard): en Cypher, `$projectId` debe coincidir con el `projectId` de los nodos en ese grafo.
...
| Preguntas abiertas en lenguaje natural ("¿cómo funciona X?", "explica el flujo de Y") | **`ask_codebase`** | Ingest u orchestrator (grafo + archivos físicos). Opcional **`scope`**, **`twoPhase`**, **`responseMode`**. **`evidence_first`** → JSON **MDD 7 secciones** (LegacyCoordinator); con orchestrator `mdd-evidence` interno. Requiere INGEST_URL + LLM; orchestrator + **INTERNAL_API_KEY**. |
