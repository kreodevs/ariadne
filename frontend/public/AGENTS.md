...
| Preguntas abiertas en lenguaje natural ("¿cómo funciona X?", "explica el flujo de Y") | **`ask_codebase`** | Ingest u orchestrator (grafo + archivos físicos). Opcional **`scope`**, **`twoPhase`**, **`responseMode`**. **`evidence_first`** → JSON **MDD 7 secciones** (LegacyCoordinator); con orchestrator `mdd-evidence` interno. Requiere INGEST_URL + LLM. |
