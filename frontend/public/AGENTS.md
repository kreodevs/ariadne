...
- Pasa **`projectId`** a las herramientas para evitar ambigüedad (ej. un componente `Header` en Legacy y otro en Moderno). Suele valer **ID de proyecto** (Ariadne) o **ID de repo** (`roots[].id`); el MCP/ingest resuelve con fallback en **file** y **chat**. **Excepciones:** `get_project_analysis` usa `POST /repositories/:id/analyze` → el parámetro debe ser **`roots[].id` del repo** a analizar (no el `id` del proyecto Ariadne). `get_modification_plan`: preferir `roots[].id` en multi-root (ver fila de la tabla).
...
**Proyecto por nombre:** Ejecuta `list_known_projects` y localiza el elemento por `name`. Para **`get_project_analysis`**, usa el **`roots[].id`** del repo que quieras analizar (no basta con el `id` del proyecto Ariadne). Para **`get_modification_plan`** en multi-root, **`roots[].id`** del repo donde está el código.
