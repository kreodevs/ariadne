...
- **Repos Nest/API (p. ej. `smile-nest`)**: el corte usa sobre todo `:Component` y **RENDERS**; Nest se indexa como **:NestService / :NestController** y **CALLS** entre **:Function**. **`@Roles()`** en controladores genera nodos **:AccessRole** y relación **ALLOWS_ACCESS_ROLE** (MVP ingest); no aparece en el grafo de componente. El aviso de aristas ausentes suele ser **normal** en API sin JSX; para acoplamiento usar **C4**, índice de repo o Cypher sobre Nest/Function/AccessRole.
