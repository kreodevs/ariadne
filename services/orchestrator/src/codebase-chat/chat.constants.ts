...
→ OPCION B (TypeORM): nodos :Model con m.source = 'typeorm'. **No uses** (File)-[:CONTAINS]->(Component) con "Entity" en el nombre: Component es UI/React. Listado sin migraciones:
\`\`\`cypher
MATCH (m:Model)
...
ORDER BY m.path
\`\`\`
(Monorepo un repo: AND m.repoId = $repoId. Nodos viejos: OR m.path CONTAINS ".entity".)
...
export const EXPLORER_TOOLS_ALL = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Obtiene conteos y listados completos de nodos indexados (File, Component, Function, Route, Model, …), sin LIMIT por defecto. Muestra acotada solo vía API graph-summary?full=0.',
