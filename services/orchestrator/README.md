...
- **Nodo `retrieve`:** ReAct con tools (OpenAI) ejecutando **solo datos** vía ingest: `POST /internal/repositories/:id/retriever-tool` (Cypher, semantic, graph summary, file content).
...
- `OPENAI_API_KEY` — Necesaria para ask_codebase y para revisión/tests en workflow full.
- `ORCHESTRATOR_LLM_MODEL` / `CHAT_MODEL` — Modelo chat (default `gpt-4o-mini`).
