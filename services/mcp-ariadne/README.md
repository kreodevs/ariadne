...
- **ask_codebase** — Preguntas en NL; ingest u orchestrator. **Si omites `responseMode`**, el MCP envía **`raw_evidence`** + **`deterministicRetriever: true`** (mismo retrieve barato que The Forge «evidencia bruta»). Opcionales: **`scope`**, **`twoPhase`**, **`responseMode`** (`default` \| **`evidence_first`** \| **`raw_evidence`**), **`deterministicRetriever`** (solo `raw_evidence`; default **true**, pon **false** para ReAct+LLM en retrieve). **`evidence_first`** → **`answer`** JSON MDD (7 claves). Timeouts: **`MCP_ASK_CODEBASE_TIMEOUT_MS`** (default 300s, 900s con `raw_evidence`). Requiere INGEST_URL; con orchestrator **`INTERNAL_API_KEY`**. **Routing:** preferir herramientas puntuales si ya conoces path/símbolo; en multi-root **`scope`** o `roots[].id` como `projectId`. Detalle: `docs/notebooklm/mcp_server_specs.md` → **Tool: `ask_codebase`**.
