...
- **ask_codebase** — Preguntas en NL; ingest u orchestrator (Coordinator → grafo + archivos físicos). Opcionales: **`scope`**, **`twoPhase`**, **`responseMode`**: `default` = prosa; **`evidence_first`** = campo **`answer`** como string JSON MDD (7 claves) para The Forge/LegacyCoordinator (orchestrator vía **mdd-evidence** si aplica). Requiere INGEST_URL y LLM; con orchestrator también **`INTERNAL_API_KEY`** (mdd-evidence).
