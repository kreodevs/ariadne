...
- **parser.ts** — Tree-sitter: TS/JS/JSX, componentes, Nest, rutas React Router (`RouteInfo.enclosingComponent`), etc. Salida temprana para MD/MDX Storybook y markdown de proyecto (`:StorybookDoc` / `:MarkdownDoc` vía producer). CSF: **`storybook-csf-ast.ts`** en `*.stories.*`.
